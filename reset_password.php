...
if(isset($_GET['email']) and isset($_GET['key']))
{
	...
	{
		echo "<script>alert('Invalid email');window.open('index.php','_self');</script>";
		die();
	}
	$row = mysqli_fetch_array($result);
	if($row['reset_token'] != $token)
	{
		echo "<script>alert('Reset token invalid');window.open('index.php','_self');</script>";
		die();
	}
}
if(isset($_POST['reset-pass']))
{
		...
		$qr = "UPDATE `user_details` SET `pass`='$pass', `reset_token`=NULL, `reset_complete`='Yes' WHERE email='$email'";
			...
			<div class="col-md-6 col-md-offset-3" >
				<div class="panel panel-login" >
					...
					<div class="panel-body">
						<div class="row">
							<div class="col-lg-12">
								<form id="login-form" action="reset_password.php" method="post" role="form" style="display: block;">
									<div class="form-group">
										<input type="email" name="email" id="email" tabindex="0" class="form-control" placeholder="" value="<?php echo $email; ?>" style="display: none;" required>
										<input type="password" name="pass" id="pass" tabindex="1" class="form-control" placeholder="Password" value="" required>
										<input type="password" name="cpass" id="cpass" tabindex="2" class="form-control" placeholder="Confirm Password" value="" required>
									</div>
									...
									<div class="form-group">
										<div class="row">
											<div class="col-sm-6 col-sm-offset-3">
												<input type="submit" name="reset-pass" id="reset-pass" tabindex="4" class="form-control btn btn-login" value="Reset password" style="background-color:#e22727">
											</div>
										</div>
									</div>
								</form>
